...
FO = fit(sondeAbsHumReal(:),absHumReal(:),'poly1');
else
    FO = nan;
end
...
hold off
set(gca, 'YDir','normal')
title('Corrected WV MPD-sonde comparison')
...
for iii = 1:size(Sonde.sonde_ind,2)
absHum(:,iii) = diag(AbsHum0m(:,Sonde.sonde_ind(:,iii)));
end
if ~isempty(Sonde.sonde_ind)
absHumReal = absHum(~isnan(absHum) & absHum>0 & Sonde.AbsHum>0);
sondeAbsHumReal = Sonde.AbsHum(~isnan(absHum) & absHum>0 & Sonde.AbsHum>0);
...
hold off
set(gca, 'YDir','normal')
title('Zero order WV MPD-sonde comparison')
xlabel('WV retrieval (g/m^3)')
ylabel('Sonde (g/m^3)')
...
figure(7825)
for jj = 1:size(Sonde.sonde_ind,2)
    %wvComparison(:,jj) = (Sonde.AbsHum(:,jj)-diag(AbsHumm(:,Sonde.sonde_ind(:,jj))))./Sonde.AbsHum(:,jj)*100;
    wvComparison(:,jj) = (Sonde.WV_sonde(:,jj)-diag(N_wvm(:,Sonde.sonde_ind(:,jj))))./Sonde.WV_sonde(:,jj)*100;
end
if isempty(Sonde.sonde_ind)
    wvComparison = nan;
end

meanWV = mean(wvComparison,2,'omitnan');
stdWV = std(wvComparison,0,2,'omitnan');
numberNonNan = sum(~isnan(wvComparison(:,:)),2);
numberNonNan = 1;
stdWV = stdWV./numberNonNan;
plot(meanWV,Range.rkm,'r','linewidth',2)
hold on
plot(meanWV+stdWV,Range.rkm,'--r')
plot(meanWV-stdWV,Range.rkm,'--r')

for jj = 1:size(Sonde.sonde_ind,2)
    %wvComparison(:,jj) = (Sonde.AbsHum(:,jj)-diag(AbsHum0m(:,Sonde.sonde_ind(:,jj))))./Sonde.AbsHum(:,jj)*100;
    wvComparison(:,jj) = (Sonde.WV_sonde(:,jj)-diag(N_wv0m(:,Sonde.sonde_ind(:,jj))))./Sonde.WV_sonde(:,jj)*100;
end
if isempty(Sonde.sonde_ind)
    wvComparison = nan;
end
...
numberNonNan = 1;
stdWV = stdWV./numberNonNan;
plot(meanWV,Range.rkm,'b','linewidth',2)
plot(meanWV+stdWV,Range.rkm,'--b')
plot(meanWV-stdWV,Range.rkm,'--b')
ylabel('Range (km)')
xlabel('\Delta(WV_{Sonde}-WV_{MPD})%')
legend('MeanCorrected','Std','std','mean 0','std','std')
hold off
xlim([-20 20])
ylim([0 4])
grid on
