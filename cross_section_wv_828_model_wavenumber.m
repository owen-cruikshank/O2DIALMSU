function [cross_section] = cross_section_wv_828_model_wavenumber(T,P,nu_Range)
%File: cross_section_wv_828_model_wavenumber.m
%Date: 02/14/2022
...
cross_section = zeros(rL,tL,length(nu_Range));%preallocate 
...
for i = 1:length(O2_parameters)                     %loop over all line parameters
    ...
    if nu_O2 >= nu_Range(:,:,1) && nu_O2 <= nu_Range(:,:,end) && S0_O2 * 100 > strength_threshold %Do not compute if line is out of specified wavnumber range
        ...
        gamma_D_T = (nuShifted/c).*sqrt(2*kB*T*log(2)/mwv); %[1/m](t x r) Dopper lineshape due to temperature
...
%         sigma =  Voight .* K ;                                          %[m^2/molecule](t x r) Voight absorption cross section
        ...
        cross_section = (y/pi).*integralV .* K+ cross_section;              %add on to previous cross_section
