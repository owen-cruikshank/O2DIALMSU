...
%%%imAlpha(~cloud_SDm(1:ind_km_max,:))=0; %Set transperency so that cloud mask can be seen
...
%imagesc(datenum(Time.date_ts),Range.rkm(1:ind_km_max),Temperature.T_finalm(1:ind_km_max,:)-273.13,colorLimits) %Plot
%imagesc(Time.thr,Range.rkm(1:ind_km_max),Temperature.T_finalm(1:ind_km_max,:)-273.13,colorLimits) %Plot
hold on
for ii = 1:size(Sonde.sonde_ind,2)
    ...
    %plot(Time.thr(Sonde.sonde_ind(:,ii)), Range.rkm,'--k')
end
...
%imAlpha(cloud_SDm(1:ind_km_max,:) ~= 1)=0; %Set transperency so that cloud mask can be seen
%%%imAlpha(~cloud_SDm(1:ind_km_max,:))=0; %Set transperency so that cloud mask can be seen
imAlpha(mask(1:ind_km_max,:) ~= 1)=1; %Set transperency so that cloud mask can be seen
%colorLimits = [min(Temperature.T_finalm(ind_km_min:ind_km_max,4:end-4)-1,[],'all'), max(Temperature.T_finalm(ind_km_min:ind_km_max,4:end-4),[],'all')]; % Set color limits to only include data -1
%colorLimits=[-20 10];
colorLimits=[-25 20];
%colorLimits=[0 30];
imagesc(datenum(Time.date_ts),Range.rkm(1:ind_km_max),temp(1:ind_km_max,:)-273.13,'AlphaData',imAlpha,colorLimits) %Plot
%imagesc(datenum(Time.date_ts),Range.rkm(1:ind_km_max),Temperature.T_finalm(1:ind_km_max,:)-273.13,colorLimits) %Plot
%imagesc(Time.thr,Range.rkm(1:ind_km_max),Temperature.T_finalm(1:ind_km_max,:)-273.13,colorLimits) %Plot
hold on
for ii = 1:size(Sonde.sonde_ind,2)
