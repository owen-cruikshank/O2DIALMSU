...
lambda_online = 828.1965;
lambda_offline = 828.2957;
...
parfor i = 1:length(nu)
    %for j = 1:N*N
        ...
        [~,y(i,:),~,~] = cross_section_wv_828_model(TP(:,1),TP(:,2),nu(i),0)
...
legend('y','ynew')
title('Cross Section and pca cross section')
hold off

figure()
plot(lambda,error(:,TPind-5:TPind+5))
title('% error between cross sec and pca cross sec')
...
parfor i = 1:length(nu)
    %for j = 1:N*N
        ...
        [~,yabs(i,:),~,~] = cross_section_wv_828_model(Ts,Ps,nu(i),0)
        %[a,y,~] = absorption_O2_770_model_wavenumber(TP(:,1),TP(:,2),nu,0);
...
legend('y','ynew')
title('New T and P cross section and PCA cross sec')
hold off

figure()
plot(lambda,errornew*100)
ylabel('error %')
title('New T and P cross sec and PCA cross sec error')
