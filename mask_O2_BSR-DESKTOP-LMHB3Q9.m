function [SNRm , cloud_SDm_above, cloud_SDm,o2on_SNR] = mask_O2(cloud_p_point,SNR_threshold,SD_threshold,oversample,t_avg,Counts,BGmult,Time,Range,BSR,lowAlt)
%File: mask_O2.m
%Date: 03/16/2020
...
%o2on_SNRm(o2on_SNR < SNR_threshold & Range.rm > 1000 | isnan(Counts.o2off)) = 0;% Replace low SNR points with zeros
...
%o2off_SNRm((o2off_SNR < SNR_threshold & Range.rm > 1000) | isnan(Counts.o2off) ) = 0;
...
if ~isempty(SNR_index_row)                                 % Check if any clouds exist
    SNR_index = zeros(1,length(Time.ts));                          % Initalize cloud range index matrix
...
figure(56)
subplot(2,1,1)
imagesc(Time.ts/60/60,Range.rm,o2on_SD)
hold on
xline(Time.ts(p_point)/60/60,'r');
hold off
set(gca, 'YDir','normal')
title('o2on standard deviation')
subplot(2,1,2)
imagesc(Time.ts/60/60,Range.rm,o2off_SD)
hold on
xline(Time.ts(p_point)/60/60,'r');
hold off
set(gca, 'YDir','normal')
