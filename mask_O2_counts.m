function [SNRm , cloud_SDm_above, cloud_SDm,o2on_SNR] = mask_O2(o2on,o2off,rm,ts,cloud_p_point,SNR_threshold,SD_threshold,oversample,t_avg,Counts,BGmult)
%File: mask_O2.m
%Date: 03/16/2020
...
%o2on_SNR = o2on./(sqrt(o2on)+Counts.bg_o2on);                  % Calculate shot noise SNR
o2on_SNR = sqrt(Counts.NBins).*o2on./(sqrt(o2on)+Counts.bg_o2on*BGmult);   % Calculate shot noise SNR with background and with averages
%o2on_SNR = o2on./(sqrt(o2on)+Counts.bg_o2on*BGmult);   % Calculate shot noise SNR with background and with averages
o2on_SNRm = ones(size(o2on_SNR));       % Initialize matrix
o2on_SNRm(o2on_SNR < SNR_threshold & rm > 1000 | isnan(o2off)) = 0;% Replace low SNR points with zeros

%o2off_SNR = o2off./(sqrt(o2off)+Counts.bg_o2off);
o2off_SNR = sqrt(Counts.NBins).*o2off./(sqrt(o2off)+Counts.bg_o2off*BGmult);   % Calculate shot noise SNR with background and with averages
%o2off_SNR = o2off./(sqrt(o2off)+Counts.bg_o2off*BGmult);   % Calculate shot noise SNR with background and with averages
o2off_SNRm = ones(size(o2off_SNR));
o2off_SNRm((o2off_SNR < SNR_threshold & rm > 1000) | isnan(o2off)) = 0;
...
%stdNeiborhood = true(3,t_avg);                                % NxN window arround each point
...
%         else
%             SNR_index(SNR_index_col(i))=length(rm);
        end
